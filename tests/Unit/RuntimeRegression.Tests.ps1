...
Describe 'Case event log handle regression' {
    ...
    It 'holds the log exclusively until the close runs and refuses writes afterwards' {
        ...
        # While the case log is open, a competing exclusive open fails. This is
        # the exact condition that made Pester TestDrive cleanup fail on Windows.
        $blockedWhileOpen = $false
        $competing = $null
        try {
            $competing = [System.IO.File]::Open($logPath, [System.IO.FileMode]::Open, [System.IO.FileAccess]::Read, [System.IO.FileShare]::None)
        }
        catch { $blockedWhileOpen = $true }
        if ($null -ne $competing) { $competing.Dispose() }
        $blockedWhileOpen | Should -BeTrue
