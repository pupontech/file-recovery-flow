...
Describe 'Entry point case creation wiring' {
    ...
    It 're-proves the exact candidate folder before the claim marker can be written' {
        ...
        # proves the nearest existing ancestor there (the destination root, which
        # this fixture answers separately) and proves the exact candidate folder at
        # the claim stage: the proof and the fresh resolution inside its separation
        # check both resolve that one path. The assertion is therefore that the
        # candidate path is proven at the claim stage, not a call count tied to one
        # implementation of the callback.
        $state.CandidateResolutions | Should -BeGreaterOrEqual 2
        @($state.CandidatePaths | Sort-Object -Unique).Count | Should -Be 1
        [string]$state.CandidatePaths[$state.CandidateResolutions - 1] | Should -Be $state.CandidatePaths[0]
        ([System.IO.Directory]::GetFileSystemEntries($destinationPath)).Count | Should -Be 1
        $candidateFolder = [string]$state.CandidatePaths[0]
