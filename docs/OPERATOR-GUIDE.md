...
| `-ResumeJobPath <path>` | Resume an existing case folder instead of starting a new one. The folder must exist and hold its claim marker and state; the case lock, the durable state, the log history, the source and destination identity, and the capacity are all re-validated before anything is written. A case with an open or running attempt is first recorded as interrupted-unknown and then presented for review; nothing is relaunched and no completed stage is repeated. `-SourcePath`, `-DestinationPath`, and `-ClientName` cannot be combined with it. |
